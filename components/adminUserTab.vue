    ...
    <div>
        <div>
          <div class="float-box float-right pb-1">
            <b-btn variant="outline-success" @click.prevent="openAddUserModal"><i class="fa fa-plus"></i> Add user</b-btn>
          </div>
        </div>
        <div v-if="users_list.length > 0">
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr v-for="(item, index) in users_list" :key="index">
                      <td onMouseOver="this.style.color='#0091ff'" onmouseleave="this.style.color='#70e5ff'" style="color: #70e5ff; font-size: 17pt; cursor: pointer;" @click.prevent="openAddModuleModal(item)">{{item.Name}}</td>
                      <td style="font-size: 17pt;">{{item.Email}}</td>
        ...
        <b-modal id="deleteUserModal" hide-footer hide-header>
          <h1 style="font-size: 16px;" class="p-3 text-center">Do you really want to delete the user?</h1>
        ...
        <b-modal id="addUserModal" hide-footer no-close-on-backdrop :title="addOrEdit == 'add' ? 'Create User' : 'Update User'">
          <div class="wrapper text-center">
            <b-form-input placeholder="Name" v-model="add_user_name" style="width: 100%;" class="p-3 item mb-2"></b-form-input>
            <b-form-input v-if="addOrEdit == 'add'" type="email" placeholder="Email" v-model="add_user_email" style="width: 100%;" class="p-3 item mb-2"></b-form-input>
            <b-form-input v-if="addOrEdit == 'add'"  placeholder="Password" type="password" v-model="add_user_password" style="width: 100%;" class="p-3 item mb-2"></b-form-input>
            <b-form-input v-if="addOrEdit == 'add'"  placeholder="Confirm Password" type="password" v-model="add_user_c_password" style="width: 100%;" class="p-3 item mb-2"></b-form-input>
...
export default {
    ...
    methods: {
      ...
      async OpenDeleteModal(user) {
        try {
          const resp = await this.$axios.post(`/user/delete`, { id: user.id, sure: null})
      ...
      async deleteUser() {
        try {
          const resp = await this.$axios.post(`/user/delete`, { id: this.selectedUserForDelete, sure: 'YES'})
          this.$bvToast.toast( resp.data.message , {
            variant: 'danger',
            solid: true
          })
      ...
      async addUpdateUsers() {
        try {
          if (this.addOrEdit == 'add') {
            ...
            const resp = await this.$axios.post('/user/update', payload)
